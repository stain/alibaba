...
public class MetaTest extends TestCase {
	...
	/** species namespace. */
	private static final String SPC = "http://example.org/species#";

	/** Directory used to store files generated by this test case. */
	private static File targetDir;

	public void testSpecies() throws Exception {
		File jar = createFile("species.jar");
		OntologyConverter converter = createConventer();
		converter.addRdfSource(find("/ontologies/species-ontology.owl"));
		converter.bindPackageToNamespace("concepts.species", SPC);
		converter.init();
		converter.createClasses(jar);
		assertTrue(jar.isFile());
		assertEquals(2, countClasses(jar, ".java"));
		assertEquals(2, countClasses(jar, ".class"));
	}
